...
"""
from tkinter import *
#from bookshelfBackend import Database
from bookshelfMYsql import Database 
...
class Window(object):
    ...
    def search_command(self):
        self.list1.delete(0,END)
        for row in bdb.search(self.title_text.get(),self.author_text.get(),self.year_text.get(),self.isbn_text.get()):
            self.list1.insert(END,row)

    def add_command(self):
        if (len(self.title_text.get()) > 0 and len(self.author_text.get()) > 0 
            and len(self.year_text.get()) > 0 and len(self.isbn_text.get()) > 0):
            bdb.insert(self.title_text.get(),self.author_text.get(),self.year_text.get(),self.isbn_text.get())
            self.list1.delete(0,END)
            self.list1.insert(END,(self.title_text.get(),self.author_text.get(),self.year_text.get(),self.isbn_text.get()))

    def delete_command(self):
        bdb.delete(selected_tuple[0])

    def update_command(self):
        if (len(self.title_text.get()) > 0 and len(self.author_text.get()) > 0 
            and len(self.year_text.get()) > 0 and len(self.isbn_text.get()) > 0):
            bdb.update(selected_tuple[0],self.title_text.get(),self.author_text.get(),self.year_text.get(),self.isbn_text.get())
